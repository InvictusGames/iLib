...
public class CC {
    ...
    public static ChatMessage createFancyUsage(String commandLabel, String arguments, String description,
                                               UsageFlag... flags) {
        ChatMessage usage =
                new ChatMessage("Usage: ").color(ChatColor.RED).hoverText(ChatColor.YELLOW + description);
        usage.add("/" + commandLabel + " ").color(ChatColor.GRAY).hoverText(ChatColor.YELLOW + description);
        usage.add(arguments).color(ChatColor.AQUA).hoverText(ChatColor.YELLOW + description);

        if (flags.length == 0) {
            return usage;
        }
        for (int i = 0; i < flags.length; i++) {
            UsageFlag flag = flags[i];
            if (i == 0) {
                usage.add(" [").color(ChatColor.AQUA).hoverText(ChatColor.YELLOW + flag.getDescription());
                usage.add(flag.getFlag()).color(ChatColor.AQUA).hoverText(ChatColor.YELLOW + flag.getDescription());
            } else {
                usage.add(" | " + flag.getFlag()).color(ChatColor.AQUA).hoverText(ChatColor.YELLOW + flag.getDescription());
            }
            if (i == flags.length - 1) {
    ...
    public static ChatMessage createFancyUsage(String commandLabel, String description, UsageFlag... flags) {
        ChatMessage usage =
                new ChatMessage("Usage: ").color(ChatColor.RED).hoverText(ChatColor.YELLOW + description);
        usage.add("/" + commandLabel).color(ChatColor.GRAY).hoverText(ChatColor.YELLOW + description);
        if (flags.length == 0) {

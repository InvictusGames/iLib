package cc.invictusgames.ilib.messages.image;
...
public class ImageMessage {
    ...
    private final ChatMessage[] lines;
    ...
    public ImageMessage(ChatMessage... imgLines) {
        lines = imgLines;
    }

    public ImageMessage(String... imgLines) {
        this.lines = new ChatMessage[imgLines.length];
        for (int i = 0; i < imgLines.length; i++)
            this.lines[i] = new ChatMessage(imgLines[i]);
    }

    public ImageMessage appendText(ChatMessage... text) {
        for (int y = 0; y < lines.length; y++) {
            if (text.length > y) {
                lines[y].add(" ").add(text[y]);
            }
        }
        return this;
    }
    ...
    private ChatMessage[] toImgMessage(ChatColor[][] colors, char imgchar) {
        ChatMessage[] lines = new ChatMessage[colors[0].length];
        for (int y = 0; y < colors[0].length; y++) {
            lines[y] = new ChatMessage("");
            ...
            lines[y].add(line.toString() + ChatColor.RESET);
        }
